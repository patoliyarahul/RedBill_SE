...
extension ManageListingVC {
    ...
    func indexOfCategory(_ id: Int) -> Int {
        ...
        let indexK = k.index {
            if let dic = $0 as? Dictionary<String,Any> {
    ...
    func setPickerData() {
        if appDelegate.arrCategory.count > 0 {
            if self.isEdit {
                ...
                self.index = Int((appDelegate.arrCategory[0][API_param.Product.category_id] as! String))!
                self.setPicker(0)
            }
        }else {
    ...
    func setPicker(_ index:Int) {
        ...
        txtCategory.text = appDelegate.arrCategory[index][API_param.Product.name] as? String
    }
...
extension ManageListingVC {
    ...
    @IBAction func btnEditStatusClicked(_ sender: Any) {
        ...
        func updateStatus_Service(_ statusCode: String){
            ...
            let params: Parameters = [API_param.Product.product_id : "\(dict[API_param.Product.product_id]!)",
                API_param.Product.status: statusCode] 
    ...
    @IBAction func btnSubmiteClicked(_ sender: Any) {
        ...
        if self.arrImage.count == 0 {
            Utils.showAlert("Error", message: MESSAGES.photo_empty, controller: self)
            return
        }
                ...
                case .success(let upload, _, _):
                    upload.responseJSON { response in
                        ...
                        if let json = response.result.value as? Dictionary<String, Any> {
                            ...
                            Utils.showAlert("Error", message: "\(response.result)", controller: self)
                        }
                    }
                case .failure(let encodingError):
                    print(encodingError)
                    Utils.showAlert("Error", message: "\(encodingError)", controller: self)
                }
            }
        }
    }
