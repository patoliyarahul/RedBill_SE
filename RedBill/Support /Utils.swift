...
enum DeviceType: String
{
    ...
    case IphonePlusDevice = "iPhone6+"
    case IPhoneX = "iPhoneX"
}
// MARK:- enum for offerStatus
...
class Utils: NSObject {
    ...
    class func downloadImage(_ url: String, imageView: UIImageView) {
        ...
        if let imageCache2 = imageDownloader.imageCache {
            let urlRequest = URLRequest(url: URL(string: finalUrl)!, cachePolicy: .reloadIgnoringCacheData, timeoutInterval: 0) // 30
    ...
    class func formetPhoneNumber(text1:String , range:NSRange,textFeild:UITextField,txtString:String) -> Bool
    {
        ...
        }else if text1.count == 4{
            
            textFeild.text = textFeild.text! + ") "
            
        }else if text1.count == 9{
            
            textFeild.text = textFeild.text! + "-"
            
        }else if text1.count > 13{
            
            return false
        }
        return true
    }
    //MARK: Detect device screen
    class func whichDevice() -> String
    {
        if UIDevice().userInterfaceIdiom == .phone
        {
            ...
            {
            case 1136:
                return DeviceType.Iphone5.rawValue
            case 1334:
                return DeviceType.Iphone6.rawValue
            case 2208:
                return DeviceType.IphonePlusDevice.rawValue
            case 2436:
                return DeviceType.IPhoneX.rawValue
            default:
                return ""
            }
        }
...
func addAttributes(text:String,range:NSRange,fontName:String,color:UIColor,fSize:Int) -> NSMutableAttributedString
{
    let multipleAttributes: [String : Any] = [
        NSFontAttributeName :UIFont.init(name:fontName, size:CGFloat(fSize))!,NSForegroundColorAttributeName:color]
...
func getAttributedTextWithSpacing(text: String) -> NSMutableAttributedString {
    ...
    attributedAddress.addAttribute(NSParagraphStyleAttributeName, value: mutableParagraphStyle, range: NSMakeRange(0, text.count))
...
func getHiddenCardNo (cardNumber: String) -> String {
    ...
    let removedSpaceStr = paymentMethodStr.replacingOccurrences(of: " ", with: "")
...
// MARK:- find which Status From Offer

func whichOfferStatus(status: offerStatus) -> String
{
    switch status
    {
    case .ACTIIVE:
